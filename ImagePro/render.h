...
namespace imagepro
{
	...
	private:
		...
		std::string OpenSaveFileDialog()
		{
			...
			{
				//std::wcout << L"Saving to file: " << ofn.lpstrFile << std::endl;
		...
		int Init()
		{
			...
			/*
			int widthW, heightW;
			glfwGetFramebufferSize(mp_Window, &widthW, &heightW);
			float aspect = (float)widthW / heightW;

			glm::mat4 Projection = glm::perspective(glm::radians(45.0f), aspect, 0.0001f, 100.0f);
			glm::mat4 View = glm::lookAt(
				glm::vec3(0, 0, 2.0f), // Camera position
		...
		void MainLoop()
		{
			do
			{
				...
				//
				{
					...
					glm::mat4 View = glm::lookAt(
						glm::vec3(0.0f, 0.0f, 1.0f),   // Camera position: 2 units forward
				...
				if (showAboutDialog)
				{
					...
					float windowSz = 330;
					ImGui::SetNextWindowPos(ImVec2(screenWidth * 0.5 - windowSz * 0.5, 40));
				...
				if (ImGui::BeginMainMenuBar())
				{
					...
					{
						if (ImGui::MenuItem("Open a New Image", "Ctrl+O"))
						{
							OpenFileDialog();
						}

						if (ImGui::MenuItem("Save Current Image...", "Ctrl+O"))
						{
						...
						if (ImGui::MenuItem("Reset Image Options Values", "Ctrl+S"))
						{
							ResetSliderValues();
						}
					...
					if (ImGui::BeginMenu("Window"))
					{
						...
						ImGui::Separator();
						if (ImGui::MenuItem("Change Background", "Ctrl+B")) 
						{
